...
class DatasetManager:
    ...
    def prepare_features_and_split(self, batch_size, val_size):
        """Split trainval into train/val randomly, encode sequences, use raw labels."""
        ...
        # Get raw labels
        self.y_train = self.train_df['half_life'].values.reshape(-1, 1)
        self.y_val = self.val_df['half_life'].values.reshape(-1, 1)
        self.y_test = self.test_df['half_life'].values.reshape(-1, 1)
        
        plt.hist(self.y_train, bins=50, alpha=0.5, label="train")
        plt.hist(self.y_val, bins=50, alpha=0.5, label="val")
        plt.hist(self.y_test, bins=50, alpha=0.5, label="test")
        plt.legend()
        plt.show()
...
def main(cfg: DictConfig):
    ...
    spearman_r, _ = spearmanr(y_test.flatten(), y_pred.flatten())
    
    # Get final epoch metrics for train and validation
    final_epoch = len(hist.history['loss'])
    train_loss = hist.history['loss'][-1]
