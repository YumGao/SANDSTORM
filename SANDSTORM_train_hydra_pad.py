...
class DatasetManager:
    ...
    def load_and_filter_data(self):
        ...
        df = df[mask].copy()
        def add_random_A_to_sequences(df):
        # 确保 'sequence' 列存在
            if 'sequence' not in df.columns:
                raise ValueError("DataFrame must contain a 'sequence' column")
            
            # 随机选择 10% 的行
            sampled_indices = df.sample(frac=0.1).index
            
            # 随机生成一个布尔数组，用于决定是否添加 'A'
            add_A = np.random.choice([True, False], size=len(sampled_indices), p=[0.5, 0.5])
            
            # 在随机选择的行的结尾添加 'A'
            df.loc[sampled_indices, 'sequence'] = df.loc[sampled_indices, 'sequence'].apply(
                lambda x: x + 'A' if np.random.choice([True, False]) else x
            )
            return df
        df = add_random_A_to_sequences(df)
    ...
    def prepare_features_and_split(self, batch_size, val_size):
        """Split trainval into train/val, encode sequences, fit scaler on train, transform val/test."""
...
def main(cfg: DictConfig):
    ...
    y_pred = dataset_manager.get_original_labels(y_pred_scaled)
    y_test_orig = dataset_manager.get_original_labels(dataset_manager.y_test)
    ...
    # Print metrics
    print(f"Final Epoch Metrics (Epoch {final_epoch}):")
